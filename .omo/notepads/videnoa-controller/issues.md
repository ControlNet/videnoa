...
- Red evidence: `cargo +1.83.0 test --locked -p videnoa-controller --test task_api authentication::direct_router_returns_typed_internal_error_when_peer_metadata_is_missing -- --exact --nocapture` panicked at `crates/controller/src/auth/boundary.rs:21` with `server requests include peer connection information`; the backtrace entered `tasks::routes::require_auth` and the test failed `0 passed; 1 failed`.
- The first post-fix repository-wide format/Clippy rerun overlapped the independently owned recovery pagination edits and reported only their temporary formatting differences plus `orchestration.rs:148` `never_loop`. No recovery file was changed by this remediation; after that work settled, the exact Rust 1.83 formatting and strict Clippy commands passed.
- Final green evidence: exact regression `1/1`, task API `6/6`, auth HTTP `8/8`, operations auth `1/1`, Rust 1.83 all-target check and strict Clippy exit 0, no-excuse audit passes all eight touched Rust files, and the final combined Controller all-target/all-feature test run passes every target with zero failures (including Task 20 `31/31` and Task 21 security `47/47`).

## 2026-09-05T09:40:31+10:00 F2 Peer Metadata Independent Verification

- Verdict: PASS with no corrective code edit. Inspected `auth/boundary.rs`, `auth/mod.rs`, `auth/http.rs`, task and operations middleware/error adapters, `task_api/authentication.rs`, and `task_api/support.rs`; missing `ConnectInfo<SocketAddr>` reaches the existing exhaustive `500 internal_error` response in task read/mutation and operations read/mutation middleware without a fabricated address or forwarding-header trust. Production `into_make_service_with_connect_info::<SocketAddr>()` remains unchanged, and the real direct peer still keys the shared login/Bearer limiter before existing cookie, Bearer, and CSRF policy.
- Rust 1.83 focused verification passed: exact missing-peer regression `1/1` with `--nocapture`; complete `task_api` `6/6`; complete `auth_http` `8/8`; exact `task14::operational_routes_require_authentication` `1/1`; exact `task14::cookie_mutations_require_same_origin_csrf_proof` `1/1`.
- Rust 1.83 quality gates passed: `cargo +1.83.0 fmt --all -- --check`; `cargo +1.83.0 check --locked -p videnoa-controller --all-targets --all-features`; `cargo +1.83.0 clippy --locked -p videnoa-controller --all-targets --all-features -- -D warnings`; and `cargo +1.83.0 test --locked -p videnoa-controller --all-targets --all-features`, including Task 20 `31/31`, Task 21 security `47/47`, task API `6/6`, and zero failures.
- Integrity checks passed: zero LSP diagnostics across all eight focused Rust files; no scoped production `panic!`, `unwrap(`, `expect(`, `TODO`, `FIXME`, or `HACK` matches; touched production modules measure 15 to 185 pure LOC; and `git diff --check` exited 0 before the append.
- No authenticated peer-metadata blocker remains. The production listener still supplies `ConnectInfo<SocketAddr>` and direct-router absence now returns a stable typed internal response instead of unwinding.

## 2026-09-05 F2 Durable Recovery Pagination

- Red startup evidence returned only `2` reconciled rows from `11` equal-timestamp nonterminal fixtures when the scan was restricted to one page. Red recurring-orchestration evidence timed out at `before_run_persistence` when an older active task occupied the only fetched row.
- The corrected persistence contract captures a scan high-water tuple and advances through strict keyset pages; both consumers also guard task IDs within one scan. No finite one-shot maximum, offset pagination, compute replay, or active ownership bypass was introduced.
- Final green evidence: focused startup `1/1`, focused orchestration starvation `1/1`, 20,000-row recovery query-plan `1/1`, Task 20 `31/31`, Rust 1.83 format/check/strict Clippy exit 0, complete Controller all-target tests zero failures, no-excuse audit passes all nine touched Rust files, and changed-file diagnostics are clean.
- No durable recovery pagination blocker remains.

## 2026-09-05 Legacy Linux p7zip Resource Safety

- p7zip 16.02 reports generic `E_FAIL` for multiple collapsed internal/I/O paths, so the hosted text alone cannot name an allocator call. The robust correction removes automatic compression-thread selection rather than retrying or swallowing exit 2; fatal diagnostics and exit propagation remain covered by `scripts/tests/package_dist_archive_test.sh`.
- Verification commands passed: `bash scripts/tests/package_dist_archive_test.sh`; `bash -n scripts/package_dist_archive.sh scripts/tests/package_dist_archive_test.sh`; `node scripts/tests/validate_ci_release_workflows.test.mjs`; real 5+ GB helper create and verify through `.001`; changed-file diagnostics; and `git diff --check`.
- No workflow setup change was justified: both smoke and release already call the shared helper after cache reclamation, and Windows packaging uses an independent PowerShell path. A new hosted exact-tip run remains required after these uncommitted changes are landed; no workflow was triggered in this task.

## 2026-09-05T10:08:39+10:00 F2 Durable Recovery Pagination Independent Verification

- Verdict: PASS after one minimal structural correction. The keyset contract consistently uses `(updated_at_ms, id)`: the high-water query selects the greatest nonterminal tuple, page queries use a strict lower and inclusive upper tuple bound, cursor advancement round-trips the persisted millisecond timestamp exactly, and the existing partial recovery index matches the predicate and order. Rows that move or appear beyond the captured upper tuple are excluded from the finite current scan and become eligible in the next recurring scan.
- Independent review found `orchestration.rs` at 274 and `recovery/reconciler.rs` at 272 nonblank, noncomment lines after pagination was added. The runtime scan loop and startup scan were moved unchanged into private focused modules; final production counts are `161`, `120`, `141`, `195`, `241`, and `37` across the six recovery production files, with no public API or behavior change.
- Termination and integrity review passed for empty pages, short final pages, full pages ending at the inclusive upper tuple, and repeated task IDs: every nonempty page advances to its final ordered tuple before processing, strict `>` guarantees cursor progress, an empty page terminates, and per-scan ID sets prevent duplicate processing after mutation. Shutdown rejects new stage permits but still advances pages, so startup deferral remains finite; recurring orchestration retains active ownership, in-memory and durable retry deadlines, pause/shutdown admission, worker-health deferral, and submission ownership.
- Rust 1.83 verification passed on final source: focused startup `1/1`; focused recurring starvation `1/1`; 20,000-row query-plan `1/1`; Task 20 `31/31`; `cargo +1.83.0 fmt --all -- --check`; all-target/all-feature check and strict Clippy; and complete Controller all-target/all-feature tests `381/381`. The no-excuse audit passed all 11 scoped Rust files, all 11 files had zero LSP diagnostics, scoped production searches found no `TODO`, `FIXME`, `HACK`, `panic!`, `unwrap`, or `expect`, and `git diff --check` exited 0.
- Files modified by this independent pass: `crates/controller/src/orchestration.rs`, new `crates/controller/src/recovery_scan.rs`, `crates/controller/src/recovery/reconciler.rs`, new `crates/controller/src/recovery/reconciler/startup.rs`, and this append-only notepad entry. Authenticated peer-metadata, packaging, plan, and Final Wave report files were not touched.

## 2026-09-05T10:26:58+10:00 F4 Legacy Linux p7zip Independent Audit

- Verdict: PASS after one minimal contract correction. The helper's only behavior change remains `-mx=5 -md=16m -mmt=1`; it does not specify `-m0`, and real archive metadata reports `Method = LZMA2:24 BCJ`, preserving automatic executable filtering. Literal invocation search found Linux CI smoke and release using the shared helper for both create and verify; the only direct workflow `7z a` paths are the unchanged Windows packaging steps.
- Three hypotheses were tested. Disk was refuted as primary cause by exact-tip `84,695,440` KiB availability versus approximately `5,276,028` KiB required and cache reclamation in both callers. Helper/layout behavior was refuted by real split `.001` and unsplit `.7z` traces, `7z t` before `7z l`, and workflow contracts. Resource pressure remains strongly supported by Intel/AMD hosted comparison and constrained-memory evidence, but is not claimed hosted-confirmed without a new exact-tip run.
- A mutation probe showed the existing fatal-path contract accepted fake p7zip status `3` while claiming exit-2 propagation. `scripts/tests/package_dist_archive_test.sh` now captures the helper status and requires exactly `2`; the same mutation now fails with `7z fatal exit 2 changed to status 3`, while the real contract passes.
- Representative CLI QA used p7zip 16.02 under `ulimit -v 450000` on a real 502 MB cuDNN library plus the actual Videnoa executable. `bash scripts/package_dist_archive.sh create ... 100m` and `verify` returned `0/0`, processed `519,518,411` bytes into three volumes, peaked at `195,624` KiB RSS with two process threads, and retained `LZMA2:24 BCJ`. Tmux QA also observed `--help` status 0 and explicit missing-archive status 1.
- Final gates passed before cleanup: `bash scripts/tests/package_dist_archive_test.sh`; `bash -n scripts/package_dist_archive.sh scripts/tests/package_dist_archive_test.sh`; `node scripts/tests/validate_ci_release_workflows.test.mjs`; zero shell LSP diagnostics; and `git diff --check`. All audit-owned archives, logs, fixture links, scripts, and tmux state are scheduled for removal; no plan, Final Wave report, Windows packaging, Rust auth, or recovery file was changed by this audit.
