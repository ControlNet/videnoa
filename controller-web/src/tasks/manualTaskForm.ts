...
export function manualTaskErrorMessage(error: ApiClientError): string {
  ...
  const outsideWorkspace = error.fieldErrors.some((fieldError) => {
    const message = fieldError.message.toLocaleLowerCase()
    return (fieldError.field === "input_path" || fieldError.field === "output_path")
      && fieldError.code === "invalid_value"
      && (message.includes("workspace") || message.includes("root") || message.includes("outside"))
  })
  if (outsideWorkspace || normalized.includes("workspace") || normalized.includes("root") || normalized.includes("outside"))
    return "The path is outside the Controller workspace. Enter an exact workspace path and create a new task."
