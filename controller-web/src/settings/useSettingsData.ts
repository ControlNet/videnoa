...
export type SettingsData = {
  ...
  readonly save: (request: SettingsUpdateRequest) => Promise<boolean>
...
export function useSettingsData(apiClient: ApiClient): SettingsData {
  ...
  async function mutate(request: () => Promise<SettingsResponse>): Promise<boolean> {
    setMutating(true)
    setActionError(null)
    try {
  ...
  return {
    ...
    setPaused: (paused) => {
      ...
      return mutate(() => apiClient.request(`api/scheduler/${paused ? "pause" : "resume"}`, {
        method: "POST",
