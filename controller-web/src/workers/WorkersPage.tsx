...
export function WorkersPage({ apiClient }: WorkersPageProps) {
  ...
  function openForm(worker: Worker | null): void {
  ...
  return (
    <div className="route-page operation-page">
      ...
      <WorkerFormDialog worker={currentEditingWorker} open={formOpen} submitting={data.mutating} actionError={data.actionError} onClose={closeForm} onCreate={data.createWorker} onUpdate={data.updateWorker} />
    </div>
  )
}
