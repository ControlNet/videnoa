...
- **Dates**: every date cell renders relative time, at every magnitude, with the absolute timestamp in `title`. Nothing falls back to an absolute format in the cell itself: these tables use auto layout, so one wide result moves every column beside it, and a value dropping out of the relative scale shifted the table by 77px -- which reads as the row flickering. Each form fits in eight characters, including `11mo ago` and, when the two clocks disagree beyond plausible skew, `in 1h`.
...
- **Refresh**: a read never clears what is rendered. A status change refuses the in-place merge and always costs one bounded page and count read, but the rows stay on screen until the answer arrives: clearing first destroyed every row, mounted loading skeletons in their place and rebuilt the table, which on fifty rows measured 58 rows destroyed for one row changing status. Loading skeletons belong to the first read of a page, not to its refresh.
...
- **Refresh**: a refresh of the task already shown swaps its fields in place; only selecting a different task falls back to the loading state. A processing task reports an update about once a second, and clearing the detail on each of those unmounted the inspector, collapsed its scroll height, and returned the operator to the top of the drawer.
- **Refresh window**: a refresh re-reads the window the operator has open, not the default page, clamped to the Controller's page maximum. Attempt history is append-only, so attempts behind the refreshed head are immutable and are carried rather than dropped. A refresh therefore never shows fewer attempts than were expanded to, and never costs more than one request.
...
- **Live health**: a `worker_updated` delta carries the whole authoritative DTO, so a worker already listed is replaced in place at no request cost and a newer version always wins. A worker the route has never listed takes one bounded refetch, because the Controller decides list order and total. Applying a delta adds, removes and moves no DOM node: only text and attributes on the affected row and the stats strip change, so a live update cannot take scroll position or focus with it.
- **Column stability**: the last-error column is pinned rather than content-sized. Its text appears and disappears on exactly the update an operator watches -- a worker coming back online clears its error -- and content-sized it collapsed by 61px, pushing every column between it and the name cell sideways. The full message stays in `title`, so the ellipsis costs nothing. Its rule is compound (`.data-table.worker-table td.worker-error`) because the shared `.data-table td` is more specific than a bare class and silently beat both the width limit and the danger colour for as long as they were written that way.
- **Live capacity**: slot usage is derived from task assignment rather than the worker row, so no worker delta is published for it. A task delta therefore takes one bounded list read while this route is mounted -- roughly one request a second per actively processing task, accepted because recomputing capacity in the client would duplicate the Controller's counting and this table exists to show these numbers.
...
- **Live scheduler**: a `scheduler_updated` delta is rendered at once and then reconciled by one bounded settings read. Both are required: the delta carries the scheduler status but no settings version, and every write on this route submits one, so a merged-only value would make the next save or pause answer 409. Reseeding the editor on that read is correct, because the scheduler fields it edits are exactly what changed.
